 ...
 * {@link com.blib.engine.domain.selection.volume.BlockSelection.GizmoMode} for the parallel block-volume case but lives
 * in its own enum because entities only support a subset of operations: there's no entity-level analogue to
 * MOVE_BLOCKS.
 * <p>
 * Driven by the workspace's T / S hotkeys when the active selection is an
 * {@link com.blib.engine.domain.selection.picking.EntitySelectable}; the same hotkeys still drive
 * {@link com.blib.engine.domain.selection.volume.BlockSelection.GizmoMode} when no entity is selected.
 * <p>

...
/**
 * Per-project recent texture imports for the texture panel. This is intentionally separate from
 * {@link com.blib.engine.modeler.ModelerRecentFiles}: opening a block model and opening that block's textures are
 * related workflows, but they should not pollute each other's Recent menus.
 */
@ApiStatus.Internal
public final class TextureRecentFiles {

    private static final Logger LOGGER = LoggerFactory.getLogger(TextureRecentFiles.class);

    private static final String FILE_NAME = "texture_recent.json";

    private static final String FIELD = "recentTextures";

    private static final int MAX_RECENT = 10;

    private static final Gson GSON = new GsonBuilder().setPrettyPrinting().create();

    private TextureRecentFiles() {}
    ...
    public static List<Entry> list(String projectName) {
        ...
        try {
            ...
            LOGGER.warn("[BLib] TextureRecentFiles.list: failed to read {}", path, e);
            return List.of();
        }
    }
    ...
    public static void recordItemOpen(String projectName, String itemId) {
        recordOpen(projectName, new Entry(Source.ITEM, itemId));
    }
    ...
    public static void recordEntityOpen(String projectName, String entityTypeId) {
        recordOpen(projectName, new Entry(Source.ENTITY, entityTypeId));
    }
    ...
    public static void recordOpen(String projectName, Entry entry) {
        ...
        try {
            ...
            LOGGER.warn("[BLib] TextureRecentFiles.recordOpen: failed to write {}", file, e);
        }
    }
}

 ...
 * <p>
 * No-op when no entity gizmo is mid-drag; the existing {@link com.blib.engine.hud.EngineSelectionRenderer} continues to
 * paint the entity's true-pose highlight underneath, so during a drag the user sees both the original (orange
 * highlight) and the destination (yellow ghost).
 */
@ApiStatus.Internal

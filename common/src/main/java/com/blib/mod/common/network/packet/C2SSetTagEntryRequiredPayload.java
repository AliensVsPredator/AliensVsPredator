...
/**
 * Client → server: flip a tag entry's {@code required} flag at {@code rawIndex} in the project's tag JSON. Used by the
 * inspector's "req / opt" badge — clicking the badge toggles the bare-string / object form vanilla tag JSON uses for
 * the two states. Disk-only; the live registry doesn't reflect the change until Reload Project. Server echoes a fresh
 * {@link S2CTagDraftPayload} so the source view stays in sync.
 */

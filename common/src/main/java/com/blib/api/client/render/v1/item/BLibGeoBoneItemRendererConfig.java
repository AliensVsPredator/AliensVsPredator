...
/**
 * Configuration for a {@link BLibGeoBoneItemRenderer}. Identifies which bone subtree of which geo model to
 * render, where its texture lives, how to position it for each {@link net.minecraft.world.item.ItemDisplayContext},
 * and (optionally) an alternate transform set to use when the holder is using the item as a shield.
 *
 * @param geoModel            Resource location of the geo JSON to source bones from. Typically an existing
 *                            entity's geo (e.g. {@code "avp_alien:geo/entity/queen.geo.json"}) so the head
 *                            doesn't duplicate resources.
 * @param texture             Texture for the rendered bone subtree. Usually the same texture the source entity
 *                            uses, so UV layout already matches.
 * @param boneName            Name of the bone whose subtree should be rendered. Every bone whose ancestor chain
 *                            does not pass through this bone is hidden via the renderer's
 *                            {@link com.blib.api.client.render.v1.BoneVisibilityFilter}. The bone itself and
 *                            all its descendants stay visible.
 * @param idleTransforms      Transforms applied per-context when the holder is not using the item as a shield
 *                            (or always, if {@code blockingTransforms} is null). Lookup falls back to
 *                            {@link BLibTransform#IDENTITY} for any context not explicitly set.
 * @param blockingTransforms  Optional alternate transform set used when {@code isBlocking.test(stack)} returns
 *                            true (typically: holder is currently using the item as a shield). {@code null}
 *                            means use {@code idleTransforms} regardless of use state.
 * @param isBlocking          Predicate that decides per-frame whether to apply {@code blockingTransforms}.
 *                            Called only when {@code blockingTransforms} is non-null. Default checks the local
 *                            client player's use state, which is correct for first-person rendering of the
 *                            local player only.
 */

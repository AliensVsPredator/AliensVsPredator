...
/**
 * A translation / rotation / scale to apply to a {@link PoseStack}, with an optional pivot point that
 * rotation and scale are applied around. Used by {@link BLibItemTransforms} to position a
 * {@link BLibGeoBoneItemRenderer}'s rendered bone for each
 ...
 * </ol>
 * Default pivot {@code (0, 0, 0)} keeps the rotation/scale anchored to the {@code translation} point. When
 * used with {@link BLibGeoBoneItemRenderer}, the renderer separately anchors the bone's authored pivot to
 * pose-stack origin, so {@code translation=(0,0,0)} and {@code pivot=(0,0,0)} already gives rotations that
 * sweep around the bone's pivot — set {@code pivot} only when the user wants a rotation center somewhere
 * other than the bone pivot itself (e.g. nudge the rotation center to the bottom of a hat).
 *
 ...
 */
public record BLibTransform(Vector3f translation, Vector3f rotation, Vector3f scale, Vector3f pivot) {

 ...
 * <p>
 * Resolves model/texture/render-type by looking up the source mob's own
 * {@link net.minecraft.client.renderer.entity.EntityRenderer} via the limb's resolved ghost — that way variant-specific
 * textures, render-type choices (e.g. {@code entityCutoutNoCull} for mobs with thin appendages like the warden's
 * tendrils or chicken's wings), and any other state captured in the source NBT carry through. The model renderer is
 * swapped to {@link LimbEntityModelRenderer} which dispatches to a vanilla {@code ModelPart} render path or to the geo
 * subtree path depending on whether the source's renderer is BLib geo or vanilla {@code LivingEntityRenderer}.
 */
public class LimbEntityRenderer extends AzEntityRenderer<DismemberedLimbEntity> {
    ...
    public LimbEntityRenderer(EntityRendererProvider.Context context) {
        super(
            AzEntityRendererConfig.<DismemberedLimbEntity>builder(
                ...
                .setRenderType(LimbEntityRenderer::resolveRenderType)
                .setModelRenderer(
    ...
    /**
     * Mirrors the source mob's render type so cull settings, transparency, and any other render-state choices the model
     * author baked into its renderer carry over to the limb fragment. For vanilla mobs we ask the
     * {@link net.minecraft.client.model.EntityModel} (which is what {@code LivingEntityRenderer.getRenderType}
     * delegates to in the common case) — that's why a warden head limb shows tendrils on both sides, since
     * {@code WardenModel}'s implicit render type is {@code entityCutoutNoCull}. For BLib geo sources we ask the
     * source's render-type provider directly. Falls back to {@code entityCutoutNoCull} on the source's texture location
     * if neither path resolves — a safer default than {@code entityCutout} for thin / single-sided geometry.
     */
    private static RenderType resolveRenderType(DismemberedLimbEntity limb) {
        var ghost = limb.getOrCreateGhost();
        var texture = resolveTextureLocation(limb);
        ...
        if (renderer instanceof AzEntityRenderer<?> geoRenderer) {
            @SuppressWarnings({ "rawtypes", "unchecked" })
            var rawConfig = (AzEntityRendererConfig) geoRenderer.config();
            return rawConfig.getRenderType(ghost, ghost);
        }

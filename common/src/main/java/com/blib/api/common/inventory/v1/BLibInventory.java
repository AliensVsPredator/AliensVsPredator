...
public class BLibInventory {
    ...
    public static final Codec<BLibInventory> CODEC = new Codec<>() {

        @Override
        public <T> Result<BLibInventory, T> decode(CodecSchema<T> codecSchema, T input) {
            return codecSchema.getList(input).andThen(consumer -> {
                ...
                return Result.ok(BLibInventory.fromItemStacks(listBuilder.toArray(ItemStack[]::new)));
            });
        }

        @Override
        public <T> T encode(CodecSchema<T> codecSchema, BLibInventory value) {
    ...
    public static BLibInventory fromItemStacks(ItemStack[] stacks) {
        var inventory = new BLibInventory(stacks.length);

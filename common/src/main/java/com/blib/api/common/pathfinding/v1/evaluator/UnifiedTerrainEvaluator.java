...
public final class UnifiedTerrainEvaluator implements TerrainEvaluator {
    ...
    private final Long2LongOpenHashMap dropOpeningLandingScanCache;
    ...
    public UnifiedTerrainEvaluator(TerrainEvaluatorConfig config, @Nullable TerrainClassificationCache classificationCache) {
        ...
        this.dropOpeningLandingScanCache = new Long2LongOpenHashMap();
    ...
    private void clearSearchCaches() {
        ...
        dropOpeningLandingScanCache.clear();
    ...
    public int getNeighbors(PathNode node, PathNode[] neighbors) {
        return getNeighbors(node, null, neighbors);
    }
    ...
    public int getNeighbors(PathNode node, @Nullable PathNode previous, PathNode[] neighbors) {
        ...
        if (node.getTerrainType() == TerrainType.WATER && usesWaterPathfinding()) {
            return getWaterNeighbors(node, previous, neighbors);
        }
    ...
    public int getPredecessors(PathNode node, PathNode[] predecessors) {
        return getPredecessors(node, null, predecessors);
    }
    ...
    public int getPredecessors(PathNode node, @Nullable PathNode previous, PathNode[] predecessors) {
        ...
        if (features.diagonalMovement()) {
            count = appendHorizontalPredecessorCandidates(node, previous, predecessors, count, DIAGONAL_OFFSETS);
        }
    ...
    private int getGroundNeighbors(PathNode node, @Nullable PathNode previous, PathNode[] neighbors) {
        var count = appendGroundNeighbors(node, previous, neighbors, 0, CARDINAL_OFFSETS);
    ...
    private int getWaterNeighbors(PathNode node, @Nullable PathNode previous, PathNode[] neighbors) {
        var count = appendWaterNeighbors(node, previous, neighbors, 0, CARDINAL_OFFSETS);

        if (features.diagonalMovement()) {
            count = appendWaterNeighbors(node, previous, neighbors, count, DIAGONAL_OFFSETS);
        }

        if (usesWaterVerticalSwim()) {
            count = appendWaterVerticalNeighbors(node, previous, neighbors, count);
        }
    ...
    private int appendHorizontalPredecessorCandidates(
        PathNode node,
    ...
    private int appendWaterVerticalPredecessorCandidates(
        PathNode node,
        @Nullable PathNode previous,
        PathNode[] predecessors,
        int count
        ...
        return appendPredecessorCandidate(
            node,
            previous,
            predecessors,
            count,
    ...
    private int appendDropOpeningPredecessorCandidates(
        PathNode node,
        @Nullable PathNode previous,
        PathNode[] predecessors,
        int count
    ...
    private int appendPredecessorCandidate(
        ...
        if (
            candidate == null
                ...
                || isParentCandidate(previous, candidate)
                || containsNode(predecessors, count, candidate)
        ) {
            return count;
        }
    ...
    private boolean isParentCandidate(@Nullable PathNode previous, @Nullable PathNode candidate) {
    ...
    private boolean isParentNode(
        ...
        return previous != null
            ...
            && previous.getY() == y
            && previous.getZ() == z
            && previous.getTerrainType() == terrainType
            && previous.getPosture() == posture;
    }

    private int appendWaterNeighbors(
        PathNode node,
        @Nullable PathNode previous,
        PathNode[] neighbors,
        int count,
        ...
        for (var offset : offsets) {
            var groundExit = findWaterExitNeighbor(node, previous, offset[0], offset[1]);

            count = appendNeighbor(neighbors, count, previous, groundExit);

            var waterTravel = findWaterTravelNeighbor(node, previous, offset[0], offset[1]);

            count = appendNeighbor(neighbors, count, previous, waterTravel);
        }

        return count;
    }

    private int appendWaterVerticalNeighbors(
        PathNode node,
        @Nullable PathNode previous,
        PathNode[] neighbors,
        int count
    ) {
        if (isParentNode(previous, node.getX(), node.getY() + 1, node.getZ(), TerrainType.WATER, waterPosture())) {
            ...
            count = appendNeighbor(neighbors, count, previous, swimUp);
        }
        ...
        if (isParentNode(previous, node.getX(), node.getY() - 1, node.getZ(), TerrainType.WATER, waterPosture())) {
            ...
            count = appendNeighbor(neighbors, count, previous, swimDown);
        }

        return count;
    }

    private int appendGroundNeighbors(
        PathNode node,
        @Nullable PathNode previous,
        PathNode[] neighbors,
        int count,
        ...
        for (var offset : offsets) {
            count = appendGroundNeighborCandidates(node, previous, neighbors, count, offset[0], offset[1]);
        }

        return count;
    }

    private int appendDropOpeningNeighbors(
        PathNode node,
        @Nullable PathNode previous,
        PathNode[] neighbors,
        int count
        ...
        for (var offset : CARDINAL_OFFSETS) {
            var neighbor = findDropOpeningNeighbor(node, previous, offset[0], offset[1]);

            count = appendNeighbor(neighbors, count, previous, neighbor);
        }

        return count;
    }

    private @Nullable PathNode findWaterTravelNeighbor(PathNode from, @Nullable PathNode previous, int dx, int dz) {
        var x = from.getX() + dx;
        var z = from.getZ() + dz;
        if (isParentNode(previous, x, from.getY(), z, TerrainType.WATER, waterPosture())) {
            markFeatureUsed(PathfindingFeature.PARENT_EDGE_PRUNING);
        } else {
            var sameLevel = tryWaterMovementNeighbor(
                from,
                x,
                from.getY(),
                z,
                dx,
                dz,
                PathfindingFeature.WATER_PATHFINDING
            );
        ...
        if (isParentNode(previous, x, from.getY() + 1, z, TerrainType.WATER, waterPosture())) {
            markFeatureUsed(PathfindingFeature.PARENT_EDGE_PRUNING);
        } else {
            var swimUp = tryWaterMovementNeighbor(
                from,
                ...
                dx,
                dz,
                PathfindingFeature.WATER_SLOPE_SWIM
            );
    ...
    private @Nullable PathNode findWaterExitNeighbor(PathNode from, @Nullable PathNode previous, int dx, int dz) {
        ...
        var x = from.getX() + dx;
        var z = from.getZ() + dz;
        var sameLevel = tryGroundExitNeighbor(from, previous, x, from.getY(), z, dx, dz);
        ...
        for (var stepUp = 1; stepUp <= config.getMaxStepHeight(); stepUp++) {
            var steppedUp = tryGroundExitNeighbor(from, previous, x, from.getY() + stepUp, z, dx, dz);
        ...
        for (var stepDown = 1; stepDown <= config.getMaxFallDistance(); stepDown++) {
            var steppedDown = tryGroundExitNeighbor(from, previous, x, from.getY() - stepDown, z, dx, dz);
    ...
    private @Nullable PathNode tryGroundExitNeighbor(
        PathNode from,
        @Nullable PathNode previous,
        int x,
        int y,
    ...
    private int appendGroundNeighborCandidates(
        PathNode from,
        @Nullable PathNode previous,
        PathNode[] neighbors,
        int count,
        ...
        if (features.sameLevelMovement()) {
            if (isParentNode(previous, x, from.getY(), z, TerrainType.GROUND, PathPosture.STANDING)) {
                markFeatureUsed(PathfindingFeature.PARENT_EDGE_PRUNING);
            } else {
        ...
        if (features.stepUp()) {
            for (int stepUp = 1; stepUp <= config.getMaxStepHeight(); stepUp++) {
                ...
                beginEdgeAttempt(from, x, y, z, PathEdgeDebugType.STEP_UP);
                var steppedUp = tryCreateGroundNode(x, y, z);
        ...
        if (features.stepDown()) {
            ...
            if (stepDownScan.hasCandidate()) {
                if (isParentNode(previous, x, stepDownScan.y(), z, TerrainType.GROUND, PathPosture.STANDING)) {
                    markFeatureUsed(PathfindingFeature.PARENT_EDGE_PRUNING);
                    return appendNeighbor(neighbors, count, previous, findWaterEntryNeighbor(from, previous, dx, dz));
                }
    ...
    private int appendNeighbor(
        PathNode[] neighbors,
        int count,
        @Nullable PathNode previous,
        @Nullable PathNode neighbor
    ) {
        if (isParentCandidate(previous, neighbor)) {
            markFeatureUsed(PathfindingFeature.PARENT_EDGE_PRUNING);
            return count;
        }

        return appendNeighbor(neighbors, count, neighbor);
    }

    private @Nullable PathNode findDropOpeningNeighbor(PathNode from, @Nullable PathNode previous, int dx, int dz) {
        var horizontalDistance = dropOpeningHorizontalDistance();
        var x = from.getX() + dx * horizontalDistance;
        var z = from.getZ() + dz * horizontalDistance;

        var dropOpeningScan = findDropOpeningLanding(from, x, z);

        if (!dropOpeningScan.hasCandidate()) {
            return null;
        }

        var y = dropOpeningScan.y();
        ...
        beginEdgeAttempt(from, x, y, z, PathEdgeDebugType.DROP_OPENING);
        var candidate = tryCreateStepDownGroundNode(x, y, z);
        var landing = candidate.node();
    ...
    private StepDownLandingScan findDropOpeningLanding(PathNode from, int x, int z) {
        if (usesSearchCaching()) {
            markFeatureUsed(PathfindingFeature.SEARCH_CACHING);

            var key = packBlockKey(x, from.getY(), z);

            if (dropOpeningLandingScanCache.containsKey(key)) {
                return decodeStepDownLandingScan(dropOpeningLandingScanCache.get(key));
            }

            var scan = findDropOpeningLandingUncached(from, x, z);
            dropOpeningLandingScanCache.put(key, encodeStepDownLandingScan(scan));

            return scan;
        }

        return findDropOpeningLandingUncached(from, x, z);
    }

    private StepDownLandingScan findDropOpeningLandingUncached(PathNode from, int x, int z) {
        var firstDropDistance = Math.max(1, config.getMaxStepHeight() + 1);

        for (var dropDistance = firstDropDistance; dropDistance <= config.getMaxFallDistance(); dropDistance++) {
            var y = from.getY() - dropDistance;
    ...
    private @Nullable PathNode findWaterEntryNeighbor(PathNode from, @Nullable PathNode previous, int dx, int dz) {
        if (!usesWaterEntry()) {
            return null;
        }
        ...
        var x = from.getX() + dx;
        var z = from.getZ() + dz;
        var sameLevel = tryWaterEntryNeighbor(from, previous, x, from.getY(), z, dx, dz);
        ...
        for (var stepDown = 1; stepDown <= config.getMaxFallDistance(); stepDown++) {
            ...
            if (isWaterEntryFallBlocked(x, y, z)) {
                break;
            }
        }
    ...
    private @Nullable PathNode tryWaterEntryNeighbor(
        PathNode from,
        @Nullable PathNode previous,
        int x,
        int y,
        ...
        if (isParentNode(previous, x, y, z, TerrainType.WATER, waterPosture())) {
            markFeatureUsed(PathfindingFeature.PARENT_EDGE_PRUNING);
            return null;
        }
    ...
    private boolean hasMovementClearance(PathNode from, PathNode to, int dx, int dz) {
        ...
        if (
            ...
            if (hasVerticalFullBlockDiagonalCorner(from, to, dx, dz)) {
                reject(PathRejectionReason.DIAGONAL_CORNER_BLOCKED, to.getX(), to.getY(), to.getZ());
                return false;
            }
        }
    ...
    private boolean hasVerticalFullBlockDiagonalCorner(PathNode from, PathNode to, int dx, int dz) {
        return hasAxisAlignedVerticalFullBlockCorner(from, to, dx, dz)
            || (dx != 0 && dz != 0 && hasTopHorizontalAxisFullBlockCorner(from, to, dx, dz));
    }

    private boolean hasAxisAlignedVerticalFullBlockCorner(PathNode from, PathNode to, int dx, int dz) {
        if ((dx == 0) == (dz == 0)) {
            return false;
        }

        // Reject X/Y or Z/Y corner cuts such as BX/AB, where the two full blocks pinch the diagonal descent/ascent.
        var low = from.getY() < to.getY() ? from : to;
        var high = from.getY() < to.getY() ? to : from;
        var footprintWidth = footprintCellWidth();

        if (dx != 0) {
            return hasAxisAlignedVerticalXFullBlockCorner(low, high, footprintWidth);
        }

        return hasAxisAlignedVerticalZFullBlockCorner(low, high, footprintWidth);
    }

    private boolean hasAxisAlignedVerticalXFullBlockCorner(PathNode low, PathNode high, int footprintWidth) {
        var lowCeilingX = low.getX() < high.getX() ? low.getX() : low.getX() + footprintWidth - 1;
        var highFloorX = high.getX() > low.getX() ? high.getX() + footprintWidth - 1 : high.getX();
        var minZ = Math.max(low.getZ(), high.getZ());
        var maxZ = Math.min(low.getZ() + footprintWidth - 1, high.getZ() + footprintWidth - 1);

        for (var y = low.getY(); y < high.getY(); y++) {
            for (var z = minZ; z <= maxZ; z++) {
                if (hasAxisAlignedVerticalCornerPair(lowCeilingX, y + 1, z, highFloorX, y, z)) {
                    return true;
                }
            }
        }
    ...
    private boolean hasAxisAlignedVerticalZFullBlockCorner(PathNode low, PathNode high, int footprintWidth) {
        var lowCeilingZ = low.getZ() < high.getZ() ? low.getZ() : low.getZ() + footprintWidth - 1;
        var highFloorZ = high.getZ() > low.getZ() ? high.getZ() + footprintWidth - 1 : high.getZ();
        var minX = Math.max(low.getX(), high.getX());
        var maxX = Math.min(low.getX() + footprintWidth - 1, high.getX() + footprintWidth - 1);

        for (var y = low.getY(); y < high.getY(); y++) {
            for (var x = minX; x <= maxX; x++) {
                if (hasAxisAlignedVerticalCornerPair(x, y + 1, lowCeilingZ, x, y, highFloorZ)) {
                    return true;
                }
            }
        }
    ...
    private boolean hasAxisAlignedVerticalCornerPair(
        int lowCeilingX,
        int lowCeilingY,
        int lowCeilingZ,
        int highFloorX,
        ...
        if (
            !isFullCollisionBlock(lowCeilingX, lowCeilingY, lowCeilingZ)
                || !isFullCollisionBlock(highFloorX, highFloorY, highFloorZ)
        ) {
            return false;
        }

        recordBlockingBlock(lowCeilingX, lowCeilingY, lowCeilingZ, PathRejectionReason.DIAGONAL_CORNER_BLOCKED);
        recordBlockingBlock(highFloorX, highFloorY, highFloorZ, PathRejectionReason.DIAGONAL_CORNER_BLOCKED);
        return true;
    }
    ...
    private record GroundCandidate(@Nullable PathNode node, boolean blocksFurtherStepDown) {
        ...
        private static GroundCandidate openUnsupported() {
            return new GroundCandidate(null, false);
        }

        private static GroundCandidate blocked() {
            return new GroundCandidate(null, true);
        }
    }
    ...
    private @Nullable PathNode tryCreateGroundNode(int x, int y, int z, PathPosture posture) {
        return evaluateGroundCandidate(x, y, z, posture).node();
    }

    private GroundCandidate tryCreateStepDownGroundNode(int x, int y, int z) {
        var standing = evaluateGroundCandidate(x, y, z, PathPosture.STANDING);

        if (standing.node() != null || !usesCrawling()) {
            return standing;
        }

        var crawling = evaluateGroundCandidate(x, y, z, PathPosture.CRAWLING);

        if (crawling.node() != null) {
            return crawling;
        }

        if (!standing.blocksFurtherStepDown() || !crawling.blocksFurtherStepDown()) {
            return GroundCandidate.openUnsupported();
        }

        return GroundCandidate.blocked();
    }

    private GroundCandidate evaluateGroundCandidate(int x, int y, int z, PathPosture posture) {
        ...
        if (usesTerrainPrecheck()) {
            ...
            if (rejectionReason != null) {
                ...
                return groundCandidateForRejection(rejectionReason);
            }
        ...
        return GroundCandidate.accepted(getOrCreateGroundNode(x, y, z, posture));
    }

    private GroundCandidate groundCandidateForRejection(PathRejectionReason rejectionReason) {
        if (rejectionReason == PathRejectionReason.UNSTABLE_SUPPORT) {
            return GroundCandidate.openUnsupported();
        }
    ...
    private @Nullable PathNode tryCreateSameLevelBlockBreakingGroundNode(
        ...
        if (currentLevel == null || from.getTerrainType() != TerrainType.GROUND || from.getY() != y) {
            return null;
        }
        ...
        if (!usesGroundBlockBreaking()) {
            reject(PathRejectionReason.BLOCK_BREAK_DISABLED, x, y, z);
            return null;
        }
    ...
    private @Nullable BlockBreakCandidate collectFeetBlockBreakCandidate(
        ...
        if (blockAccessor.isLiquid(state)) {
            reject(PathRejectionReason.BLOCK_BREAK_LIQUID_BLOCKED, x, y, z);
            return null;
        }

        if (!blockAccessor.isSolid(state)) {
            reject(PathRejectionReason.CANDIDATE_REJECTED, x, y, z);
            return null;
        }
        ...
        for (var x = minX; x <= maxX; x++) {
            for (var y = minY; y <= maxY; y++) {
                for (var z = minZ; z <= maxZ; z++) {
                    ...
                    if (blockAccessor.isLiquid(state)) {
                        reject(PathRejectionReason.BLOCK_BREAK_LIQUID_BLOCKED, nodeX, nodeY, nodeZ);
                        return null;
                    }
                    ...
                    if (blocks.size() >= breakConfig.maxBlocksPerEdge()) {
                        reject(PathRejectionReason.BLOCK_BREAK_LIMIT_EXCEEDED, nodeX, nodeY, nodeZ);
                        return null;
                    }
    ...
    private PathRejectionReason blockBreakRejectionReason(
        LevelReader level,
        BlockPos pos,
        BlockState state,
        PathBlockBreakingConfig breakConfig
    ) {
        if (!breakConfig.enabled() || breakConfig.maxBlocksPerEdge() <= 0) {
            return PathRejectionReason.BLOCK_BREAK_DISABLED;
        }

        var hardness = state.getBlock().defaultDestroyTime();
        if (hardness < 0.0f || hardness > breakConfig.maxHardness()) {
            return PathRejectionReason.BLOCK_BREAK_UNBREAKABLE;
        }

        if (!breakConfig.breakPolicy().canBreak(level, pos, state)) {
            return PathRejectionReason.BLOCK_BREAK_POLICY_REJECTED;
        }
    ...
    private boolean isWaterEntryFallBlocked(int x, int y, int z) {
        var posture = waterPosture();
        ...
        for (var waterX = x; waterX < x + footprintWidth; waterX++) {
            for (var waterZ = z; waterZ < z + footprintWidth; waterZ++) {
                if (!isWaterEntryFallCellPassable(waterX, y, waterZ)) {
                    return true;
                }
            }
        }
    ...
    private boolean isWaterEntryFallCellPassable(int x, int y, int z) {
        ...
        var passable = !blockAccessor.isSolid(state)
            && (!blockAccessor.isLiquid(state) || blockAccessor.isWater(state));

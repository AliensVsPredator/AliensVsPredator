...
public class ParasiteAttachmentManager {
    ...
    public void tick() {
        ...
        if (ticksAttachedToHost() < 20 * 10) {
            ...
            var damageSource = new DamageSource(parasite.level().registryAccess().registry(Registries.DAMAGE_TYPE).get().getHolderOrThrow(DamageTypes.IN_WALL));
            host.hurt(damageSource, 0.01F);

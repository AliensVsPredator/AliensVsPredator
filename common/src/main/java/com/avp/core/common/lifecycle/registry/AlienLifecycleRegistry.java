...
import com.avp.core.common.lifecycle.AlienLifecycle;
import com.avp.core.common.lifecycle.growth.AlienGrowthStageKey;
    ...
    public static AlienLifecycle register(AlienLifecycle lifecycle) {
        ...
        if (hosts == null) {
            stages.forEach(stage -> {
                // FIXME: .get()
                var lookupKey = new AlienGrowthStageKey(null, stage.from().get());
            ...
            hosts.forEach(host -> stages.forEach(stage -> {
                // FIXME: .get()
                var lookupKey = new AlienGrowthStageKey(host, stage.from().get());
                ALIEN_MATURATION_LOOKUP_MAP.put(lookupKey, stage);
            }));
        }

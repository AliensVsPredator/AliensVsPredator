package com.avp.common.constant;

public class HealthRegenConstants {

    public static final float ADOLESCENT_HEALTH_REGEN = 0.5F;

    public static final float BOILER_HEALTH_REGEN = 0.5F;

    public static final float CHESTBURSTER_HEALTH_REGEN = 0.5F;

    public static final float CRUSHER_HEALTH_REGEN = 0.5F;

    public static final float DRONE_HEALTH_REGEN = 0.5F;

    public static final float FACEHUGGER_HEALTH_REGEN = 0.5F;

    public static final float OVOMORPH_HEALTH_REGEN = 0.5F;

    public static final float PRAETORIAN_HEALTH_REGEN = 0.5F;

    public static final float PREDALIEN_ADOLESCENT_HEALTH_REGEN = 0.5F;

    public static final float PREDALIEN_CHESTBURSTER_HEALTH_REGEN = 0.5F;

    public static final float PREDALIEN_HEALTH_REGEN = 0.5F;

    public static final float PROWLER_HEALTH_REGEN = 0.5F;

    public static final float QUEEN_HEALTH_REGEN = 0.5F;

    public static final float RUNNER_HEALTH_REGEN = 0.5F;

    public static final float SPITTER_HEALTH_REGEN = 0.5F;

    public static final float WARRIOR_HEALTH_REGEN = 0.5F;

    private HealthRegenConstants() {
        throw new UnsupportedOperationException();
    }
}

...
public class AVPFabricLootTableModifier {
    ...
    public static void registerVanillaLootTableModifications() {
        LootTableEvents.MODIFY.register((resourceManager, lootManager, id, tableBuilder, source) -> {
            if (source.isBuiltin()) {
                addSimpleLootPool(BuiltInLootTables.VILLAGE_WEAPONSMITH, id, tableBuilder, AVPWeaponBlueprintItems.BLUEPRINT_M88MOD4_COMBAT_PISTOL, 20);
                ...
                addSimpleLootPool(BuiltInLootTables.STRONGHOLD_CORRIDOR, id, tableBuilder, AVPWeaponBlueprintItems.BLUEPRINT_M41A_PULSE_RIFLE, 20);
                addSimpleLootPool(BuiltInLootTables.BASTION_TREASURE, id, tableBuilder, AVPWeaponBlueprintItems.BLUEPRINT_FLAMETHROWER_SEVASTOPOL, 33);
                ...
                addSimpleLootPool(BuiltInLootTables.END_CITY_TREASURE, id, tableBuilder, AVPWeaponBlueprintItems.BLUEPRINT_M56_SMARTGUN, 10);

                // AK-47
                if (BuiltInLootTables.TRAIL_RUINS_ARCHAEOLOGY_RARE.equals(id)) {
                    var poolBuilder = LootPool.lootPool()
                        .add(
                            LootItem.lootTableItem(AVPWeaponBlueprintItems.BLUEPRINT_AK_47.get())
                        )
                        .build();
    ...
    private static void addSimpleLootPool(ResourceLocation target, ResourceLocation id, LootTable.Builder tableBuilder, GameObject<Item> itemGameObject, int weight) {
        if (!target.equals(id)) {
            return;
        }

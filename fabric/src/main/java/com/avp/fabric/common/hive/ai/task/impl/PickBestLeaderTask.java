package com.avp.fabric.common.hive.ai.task.impl;
...
import com.avp.fabric.common.hive.ai.task.HiveTask;

public class PickBestLeaderTask extends HiveTask {

    private static final int FREQUENCY = 20 * 10;

    private static final Map<EntityType<?>, Integer> LEADER_DISPOSITION_BY_TYPE = Map.ofEntries(
        Map.entry(AVPEntityTypes.DRONE, 0),
        Map.entry(AVPEntityTypes.WARRIOR, 1),
        Map.entry(AVPEntityTypes.PRAETORIAN, 2),
        Map.entry(AVPEntityTypes.QUEEN, 3)
    );
    ...
    public boolean compareEntityTypes(EntityType<?> current, EntityType<?> other) {
        var currentDisposition = LEADER_DISPOSITION_BY_TYPE.getOrDefault(current, -1);
        var contestantDisposition = LEADER_DISPOSITION_BY_TYPE.getOrDefault(other, -1);

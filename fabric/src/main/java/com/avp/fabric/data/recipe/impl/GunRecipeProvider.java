...
public class GunRecipeProvider {
    ...
    private static void createGenericGunRecipe(RecipeBuilder builder, Supplier<Item> blueprintItemSupplier, Supplier<Item> resultSupplier, boolean hasStock) {
        var shapeless = builder.shapeless()
            .withCategory(RecipeCategory.COMBAT)
            .requires(1, blueprintItemSupplier)
            .requires(1, AVPItems.BARREL)
            .requires(1, AVPItems.GRIP)
            .requires(1, AVPItems.RECEIVER);
        ...
        shapeless.into(1, resultSupplier);
    }
}

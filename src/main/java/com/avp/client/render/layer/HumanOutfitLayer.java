...
public class HumanOutfitLayer implements AzRenderLayer<MarineMob> {
    ...
    public void render(AzRendererPipelineContext<MarineMob> context) {
        ...
        if (Boolean.TRUE.equals(animatable.getEntityData().get(AbstractHumanMob.SET_GENDER))) {
            context.setVertexConsumer(context.multiBufferSource().getBuffer(RenderType.entityCutout(animatable.getOutfitManager().getMaleOutfitTexture(HUMAN_TYPE))));
        } else {
            context.setVertexConsumer(context.multiBufferSource().getBuffer(RenderType.entityCutout(animatable.getOutfitManager().getFemaleOutfitTexture(HUMAN_TYPE))));
        }
